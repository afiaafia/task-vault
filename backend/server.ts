...
const request_tail =
  request_timeout + Math.max(connections_checking_interval, api_timeout);
...
const shutdown = async (reason: string, exitCode: number): Promise<void> => {
  if (exitCode !== 0 && pendingExitCode === 0) {
  ...
  if (pendingExitCode === 0 && drainDelay > 0) {
    logSafely('info', { drainDelay }, 'Draining before closing listener');
  ...
  const forceTimer = setTimeout(() => {
    ...
    logSafely(
      ...
      'graceful shutdown timedout and forcing exit'
    );
...
const startServer = async (): Promise<void> => {
  ...
  try {
    ...
    if (listenPromise === pendingListen) {
      listenPromise = null;
    }
  }
